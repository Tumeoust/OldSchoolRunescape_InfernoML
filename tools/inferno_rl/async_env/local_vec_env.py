...
class LocalVecEnv:
    def __init__(
        ...
        self._waiting_step_started: dict[int, float] = {}
        self._waiting_reset_started: dict[int, float] = {}
    ...
    async def poll_reset_async(
        ...
        for index in indices:
            ...
            self._waiting_reset_started.pop(index, None)
    ...
    def step_async(
        ...
        for action_index, remote_index in enumerate(indices):
            ...
            future = asyncio.run_coroutine_threadsafe(
                LocalVecEnv.__step(env, action, self.reset_options), self._loop
            )
            self._waiting_steps[remote_index] = asyncio.wrap_future(
                future, loop=self._loop
            )
            self._waiting_step_started[remote_index] = time.monotonic()
    ...
    async def poll_step_async(
        ...
        for index in indices:
            ...
            self._waiting_step_started.pop(index, None)
    ...
    def debug_state(self) -> dict[str, Any]:
        ...
        return {
            ...
            "pending_reset": sorted(self._waiting_resets.keys()),
            "pending_step": sorted(self._waiting_steps.keys()),
            "pending_reset_age_seconds": {
                idx: round(now - started, 3)
                for idx, started in self._waiting_reset_started.items()
            },
            "pending_step_age_seconds": {
                idx: round(now - started, 3)
                for idx, started in self._waiting_step_started.items()
            },
    ...
    def max_pending_duration_seconds(self) -> float:
        now = time.monotonic()
        started_times = [
            *self._waiting_reset_started.values(),
            *self._waiting_step_started.values(),
        ]
